...
export default function StayHeader({ stay, onBack }: StayHeaderProps) {
  return (
    <>
      {/* Navigation Header */}
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link
          href={`/stays/update-stay?stayId=${stay.slug}`}
          ...
          <div className="flex items-center gap-1.5 mb-2">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                size={16}
                className={`${i < (stay.starRating || 0) ? 'text-amber-400 fill-amber-400' : 'text-slate-200'}`}
              />
            ))}
            <span className="text-slate-400 text-xs font-bold ml-1.5 uppercase tracking-wider font-sans">
              ({stay.starRating}-Star Standard)
            </span>
          </div>

          <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight leading-none font-sans">{stay.name}</h1>
          
          <div className="flex items-center gap-1 text-slate-500 mt-2 text-xs font-bold italic">
            <MapPin size={14} className="text-slate-400 shrink-0" />
            <span>{stay.location?.address || 'Global Destination'}</span>
          </div>
