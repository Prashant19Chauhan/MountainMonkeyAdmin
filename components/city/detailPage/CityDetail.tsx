...
import { getCityDetailSectionsApi, updateCityDetailSectionsApi } from '@/services/city.service';
...
export default function CityDetail() {
  ...
  // Fetch CMS sections when cityId is known
  const fetchCmsSections = async () => {
    if (!cityId) return;
    setCmsLoading(true);
    try {
      const data = await getCityDetailSectionsApi(cityId);
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updateCityDetailSectionsApi(cityId, customSections);
        ...
        <div className="mt-10 pb-10">
          <div className="mb-8 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-slate-500 font-medium text-sm">Add rich content blocks shown at the bottom of this city&apos;s detail page.</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
          ...
          {customSections.length === 0 ? (
            <div className="bg-white rounded-3xl border border-slate-100 p-14 text-center text-slate-400 shadow-sm space-y-3">
              ...
              <p className="text-slate-500 text-xs font-medium max-w-xs mx-auto text-center">Click &quot;Add Block&quot; to build engaging content for this city&apos;s detail page.</p>
            </div>
          ) : (
            <form onSubmit={handleSave} className="space-y-8">
              {customSections.map((section, sIdx) => (
                <div key={sIdx} className="bg-white p-8 sm:p-10 rounded-[2.5rem] border border-slate-100 shadow-sm relative text-left">
                  {/* Controls */}
