...
import { 
  getPrivacyPageSections, 
  updatePrivacyPageSections
...
export default function PrivacyManagementPage() {
  ...
  const fetchPageData = async () => {
    setLoading(true);
    try {
      const data = await getPrivacyPageSections();
      ...
      toast.error("Failed to fetch Privacy Policy page data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateSectionField = (sIdx: number, field: keyof PrivacySection, value: string) => {
  ...
  const createNewSection = () => {
    setSections([
      ...sections,
      {
        heading: 'New Privacy Section',
        content: 'Write the detailed privacy policy, data collection terms, or usage disclosures here...'
      }
    ]);
  };

  const removeSection = (sIdx: number) => {
    if (window.confirm("Are you sure you want to delete this Privacy Section?")) {
    ...
    try {
      ...
      for (const section of sections) {
        if (!section.heading.trim()) {
          toast.error("All Privacy Section headings must be filled.");
          return;
        }
      ...
      await updatePrivacyPageSections({ sections });
      toast.success("Privacy policy sections saved successfully!");
      fetchPageData();
    } catch (error: any) {
      toast.error(error.message || "Failed to save Privacy Policy sections");
    }
  };
      ...
      <div className="mb-10 flex justify-between items-end">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Privacy Policy CMS</h1>
          </div>
          <p className="text-slate-500 font-medium">Manage corporate data collection agreements, user privacy disclosures, tracking consent guidelines, and protocols.</p>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/metadata?privacy-page=privacy-page">
      ...
      >
        <form onSubmit={handlePageUpdate} className="space-y-10">
          ...
          <div className="flex justify-between items-center bg-white p-6 rounded-3xl border border-slate-100 shadow-xs">
            <div>
              <h2 className="text-xl font-black text-slate-900 uppercase tracking-wide">Privacy Sections</h2>
              <p className="text-slate-500 font-medium text-sm mt-1">Configure and sequence dynamic policy elements on the public Privacy Policy page.</p>
            </div>
            <button
              type="button"
          ...
          {sections.length === 0 ? (
            <div className="bg-white rounded-3xl border border-slate-100 p-16 text-center text-slate-400 space-y-4 shadow-sm">
              <ShieldCheck size={48} className="mx-auto text-slate-300 animate-pulse" />
              <p className="font-bold text-sm uppercase tracking-widest">No privacy sections created yet</p>
              <p className="text-slate-500 font-medium text-xs max-w-sm mx-auto">Click "Add Privacy Section" to start drafting guidelines for user tracking, cookie storage, and encryption.</p>
            </div>
          ) : (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2 max-w-xl">
                      ...
                      <input
                        ...
                        placeholder="e.g. 1. Types of Data We Collect"
                      />
                    </div>
          ...
          >
            <Save size={20} /> Save Privacy Configurations
          </button>
        </form>
      </motion.div>
    </div>
