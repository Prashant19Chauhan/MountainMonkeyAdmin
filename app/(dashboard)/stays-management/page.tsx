...
export default function StaysManagementPage() {
  ...
  const fetchPageData = async () => {
    setLoading(true);
    try {
      const data = await getStaysPageSections();
      ...
      toast.error("Failed to fetch stays page data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCustomSectionsUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updateStaysPageSections({ customSections });
      toast.success("Stays page custom content sections saved successfully!");
      fetchPageData();
    } catch (error: any) {
    ...
    <div className="p-8 max-w-7xl mx-auto min-h-screen bg-slate-50/30">
      ...
      <div className="mb-10 flex justify-between items-end">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Stays Hub Control</h1>
          </div>
      ...
      >
        ...
        <div className="flex justify-between items-center bg-white p-6 rounded-3xl border border-slate-100 shadow-xs">
          <div>
            <h2 className="text-xl font-black text-slate-900 uppercase tracking-wide">Stays Page Custom Sections</h2>
            <p className="text-slate-500 font-medium text-sm mt-1">Design and arrange layout blocks below the stays list.</p>
          </div>
          <button
            type="button"
