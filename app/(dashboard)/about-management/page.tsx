...
export default function AboutManagementPage() {
  const [hero, setHero] = useState({
    title: "About Us",
    tagline: "Learn more about our mystical Himalayan journey.",
    image: ""
  });
  ...
  const fetchPageData = async () => {
    setLoading(true);
    try {
      const data = await getAboutPageSections();
      if (data) {
        if (data.hero) {
          setHero({
  ...
  const handlePageUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updateAboutPageSections({ hero, customSections });
      ...
      <div className="mb-10 flex justify-between items-end">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">About Us CMS Control</h1>
          </div>
          <p className="text-slate-500 font-medium">Manage experiential content, FAQs, and layout blocks for the dynamic About Us public page.</p>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/metadata?about-page=about-page">
      ...
      >
        <form onSubmit={handlePageUpdate} className="space-y-10">
          ...
          <div className="bg-white p-8 sm:p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-6">
            <div>
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-500 block mb-2">
                Hero Section Configuration
              </span>
              <h2 className="text-xl font-black text-slate-900 uppercase tracking-wide">Hero Banner Layout</h2>
              <p className="text-slate-500 font-medium text-xs">Configure the top hero banner content and imagery.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    placeholder="e.g. Our Mystical Journey"
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <LayoutDashboard size={14} className="text-indigo-500" /> Hero Tagline
                  </label>
                  <textarea
                    required
                    value={hero.tagline}
                    ...
                    placeholder="Describe your organization's vision/mission..."
                  />
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <ImageIcon size={14} className="text-indigo-500" /> Hero Image URL
                  </label>
                  <input
                    type="text"
                    value={hero.image}
                ...
                {hero.image && (
                  <div className="relative aspect-video max-w-sm rounded-2xl overflow-hidden bg-slate-100 border border-slate-200">
                    <Image src={hero.image} className="w-full h-full object-cover" />
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="flex justify-between items-center bg-white p-6 rounded-3xl border border-slate-100 shadow-xs">
            <div>
              <h2 className="text-xl font-black text-slate-900 uppercase tracking-wide">Custom Content Blocks</h2>
              <p className="text-slate-500 font-medium text-sm mt-1">Design and arrange alternating details panels, galleries, and accordion FAQs.</p>
            </div>
            <button
              type="button"
          ...
          {customSections.length === 0 ? (
            <div className="bg-white rounded-3xl border border-slate-100 p-16 text-center text-slate-400 space-y-4 shadow-sm">
              ...
              <p className="text-slate-500 font-medium text-xs max-w-sm mx-auto">Click "Add Content Block" to build galleries, accordion FAQs, link pills, and descriptions dynamically.</p>
            </div>
          ) : (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      <div className="space-y-6">
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all text-sm"
                            placeholder="e.g. Over a Decade of Guided Exploration"
                          />
                        </div>
                      ...
                      <div className="space-y-6">
                        <div className="space-y-3 bg-slate-50/50 p-6 rounded-3xl border border-slate-100">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                            <LinkIcon size={14} className="text-indigo-500" /> Action Links / Pills
                          </label>
                          <div className="grid grid-cols-2 gap-2">
                            <input
                              ...
                              placeholder="Link Label (e.g. Read Our Blog)"
                            />
                            <input
                              type="text"
