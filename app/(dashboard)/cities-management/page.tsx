...
import { 
  getCitiesPageSections, 
  updateCitiesPageSections
} from '@/services/citiesPage.service';
import { 
  Globe, 
  Save, 
  Plus, 
...
export default function CitiesManagementPage() {
  const [title, setTitle] = useState("Available Cities");
  const [description, setDescription] = useState("Explore our dynamic basecamp operations and curated tour packages operating across major Himalayan cities.");
  const [sections, setSections] = useState<CitiesSection[]>([]);
  const [loading, setLoading] = useState(true);

  const fetchPageData = async () => {
    setLoading(true);
    try {
      const data = await getCitiesPageSections();
      if (data) {
        setTitle(data.title || "Available Cities");
        setDescription(data.description || "Explore our dynamic basecamp operations and curated tour packages operating across major Himalayan cities.");
        setSections(data.sections || []);
      } else {
        setSections([]);
      }
    } catch (error: any) {
      toast.error("Failed to fetch Cities page data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateSectionField = (sIdx: number, field: keyof CitiesSection, value: string) => {
  ...
  const createNewSection = () => {
    setSections([
      ...sections,
      {
        heading: 'New Custom Content Section',
        content: 'Write dynamic details, regional information, altitude preparation guidelines, or city-specific activity highlights here...'
      }
    ]);
  };
    ...
    try {
      ...
      for (const section of sections) {
        if (!section.heading.trim()) {
          toast.error("All Custom Section headings must be filled.");
          return;
        }
      ...
      await updateCitiesPageSections({ title, description, sections });
      toast.success("Cities page configurations saved successfully!");
      fetchPageData();
    } catch (error: any) {
      toast.error(error.message || "Failed to save Cities page details");
    }
  };
      ...
      <div className="mb-10 flex justify-between items-end">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Cities Page CMS</h1>
          </div>
          <p className="text-slate-500 font-medium">Manage Available Cities page layouts, custom guidelines sections below the list, and SEO metadata.</p>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/metadata?cities-page=cities-page">
      ...
      >
        <form onSubmit={handlePageUpdate} className="space-y-10">
          ...
          <div className="bg-white p-8 sm:p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-6">
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-500 block">
              Core Page Layout Configuration
            </span>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <Type size={14} className="text-indigo-500" /> Page Hero Title
                </label>
                <input
                  type="text"
                  required
                  ...
                  placeholder="e.g. Operational Mountain Hubs"
                />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <AlignLeft size={14} className="text-indigo-500" /> Hero Subtitle Description
                </label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all text-sm"
                  placeholder="Provide sub-header guidelines here..."
                />
              </div>
            </div>
          </div>

          {/* Dynamic sections card creator bar */}
          <div className="flex justify-between items-center bg-white p-6 rounded-3xl border border-slate-100 shadow-xs">
            <div>
              <h2 className="text-xl font-black text-slate-900 uppercase tracking-wide">Custom Layout Content Sections</h2>
              <p className="text-slate-500 font-medium text-sm mt-1">Configure descriptive sections, gear lists, altitude protocols, or city guides to be rendered below the cities directory.</p>
            </div>
            <button
              type="button"
            ...
            >
              <Plus size={16} /> Add Layout Section
            </button>
          </div>
          ...
          {sections.length === 0 ? (
            <div className="bg-white rounded-3xl border border-slate-100 p-16 text-center text-slate-400 space-y-4 shadow-sm">
              ...
              <p className="text-slate-500 font-medium text-xs max-w-sm mx-auto">Click "Add Layout Section" to begin drafting guidelines, regional warnings, or custom checklists for travelers.</p>
            </div>
          ) : (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2 max-w-xl">
                      ...
                      <input
                        ...
                        className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-800 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all text-sm"
                        placeholder="e.g. Travel Altitude Preparation"
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <AlignLeft size={14} className="text-indigo-500" /> Section Paragraph Content
                      </label>
                      <textarea
                        required
                        ...
                        placeholder="Write dynamic descriptions, check lists, or links here..."
                      />
                    </div>
                  </div>
                </div>
          ...
          >
            <Save size={20} /> Save Cities Configurations
          </button>
        </form>
      </motion.div>
    </div>
