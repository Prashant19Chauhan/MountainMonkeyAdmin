...
export default function PackagesManagementPage() {
  ...
  const fetchPageData = async () => {
    setLoading(true);
    try {
      const data = await getPackagesPageSections();
      ...
      toast.error("Failed to fetch packages page data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCustomSectionsUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updatePackagesPageSections({ customSections });
      toast.success("Packages page custom content sections saved successfully!");
      fetchPageData();
    } catch (error: any) {
    ...
    <div className="p-8 max-w-7xl mx-auto min-h-screen bg-slate-50/30">
      ...
      <div className="mb-10 flex justify-between items-end">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Packages Hub Control</h1>
          </div>
          <p className="text-slate-500 font-medium">Manage experiential content and FAQ accordions for the all-packages list page.</p>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/metadata?packages-page=packages-page">
      ...
      >
        ...
        <div className="flex justify-between items-center bg-white p-6 rounded-3xl border border-slate-100 shadow-xs">
          <div>
            <h2 className="text-xl font-black text-slate-900 uppercase tracking-wide">Packages Page Custom Sections</h2>
            <p className="text-slate-500 font-medium text-sm mt-1">Design and arrange layout blocks below the packages list.</p>
          </div>
          <button
            type="button"
                    ...
                    <div className="space-y-6">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          placeholder="e.g. Master Curated Himalayan Itineraries"
                        />
                      </div>
                      ...
                      <div className="space-y-3 bg-slate-50/50 p-6 rounded-3xl border border-slate-100">
                        ...
                        <div className="space-y-2">
                          <input
                            ...
                            placeholder="Question (e.g. Inclusions details?)"
                          />
                          <textarea
